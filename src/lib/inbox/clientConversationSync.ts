...
function matchesParticipant(user: User, data: SSEMessageData): boolean {
  ...
  const sameParticipant =
    user.recipientId === data.senderId || user.recipientId === data.recipientId;
  const sameAccount =
    !user.accountId || !data.accountId || user.accountId === data.accountId;
  return sameParticipant && sameAccount;
}

export function findConversationForRealtimeMessage(
  users: User[],
  data: SSEMessageData,
): User | null {
  const byConversationId = users.find(
    (user) => user.id === data.conversationId,
  );
...
export async function fetchLatestConversationMessages(
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to fetch latest Supabase messages for conversation ${conversationId}`,
    );
  }
