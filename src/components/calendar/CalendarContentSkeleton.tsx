...
const EVENT_ROW_WIDTHS = ["w-full", "w-5/6", "w-4/5"] as const;
...
function MonthCellSkeleton({ index }: { index: number }) {
  const showPrimaryEvent = index === 8 || index === 9 || index === 18 || index === 33;
  ...
  return (
    <div className="border-b border-[#F0F2F6] p-4">
      ...
      <div className="grid grid-cols-2 gap-2">
        {SIDEBAR_STAT_CARD_CLASSES.map((cardClass, index) => (
          <div
            key={index}
  ...
  return (
    <div className="border-b border-[#F0F2F6] p-4">
      ...
      <div className="space-y-2">
        {EVENT_TYPE_BADGE_CLASSES.map((badgeClass, index) => (
          <div key={index} className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className={`h-2.5 w-2.5 animate-pulse rounded-full ${badgeClass}`} />
              <div className="h-3 w-20 animate-pulse rounded bg-[#F4F5F8]" />
            </div>
            <div className={`h-4 w-5 animate-pulse rounded-full ${badgeClass}`} />
          </div>
        ))}
      </div>
    </div>
...
export default function CalendarContentSkeleton({
  ...
  return (
    <div className={`flex min-h-0 flex-1 overflow-hidden bg-white ${compact ? "" : ""}`}>
      <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
        ...
        <div className="grid min-h-0 flex-1 grid-cols-7 grid-rows-6 overflow-hidden bg-white">
          {Array.from({ length: 42 }).map((_, index) => (
            <MonthCellSkeleton key={index} index={index} />
          ))}
        </div>
      </div>
