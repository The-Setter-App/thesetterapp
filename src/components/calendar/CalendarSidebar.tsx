...
export default function CalendarSidebar({
    ...
    const miniDays = getMonthGridDays(miniMonth.getFullYear(), miniMonth.getMonth());
                                ...
                                {miniDays.slice(0, 35).map((day) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`relative inline-flex h-7 w-full items-center justify-center rounded-md text-[11px] font-medium transition-colors ${isSelected
                        ...
                        <div className="border-b border-[#F0F2F6] p-4">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                <div className="rounded-xl border border-[#F0F2F6] bg-[#F8F7FF] p-3">
                                    ...
                                    <p className="text-lg font-bold text-[#101011]">{todayEvents.length}</p>
                                    <p className="text-[10px] font-medium text-[#606266]">Total Events</p>
                                </div>
                                <div className="rounded-xl border border-[#F0F2F6] bg-emerald-50 p-3">
                                    ...
                                    <p className="text-lg font-bold text-[#101011]">{confirmedCount}</p>
                                    <p className="text-[10px] font-medium text-[#606266]">Confirmed</p>
                                </div>
                                <div className="rounded-xl border border-[#F0F2F6] bg-amber-50 p-3">
                                    ...
                                    <p className="text-lg font-bold text-[#101011]">{pendingCount}</p>
                                    <p className="text-[10px] font-medium text-[#606266]">Pending</p>
                                </div>
                                <div className="rounded-xl border border-[#F0F2F6] bg-[#F3F0FF] p-3">
                                    ...
                                    <p className="text-[10px] font-medium text-[#606266]">Pipeline</p>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="flex flex-col gap-2">
                                {Object.entries(EVENT_TYPE_CONFIG).map(([type, config]) => (
                                    <div key={type} className="flex items-center justify-between">
                                        ...
                                        <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-semibold ${config.bgClass} ${config.textClass} ${config.borderClass}`}>
    ...
    return (
        <div className="flex flex-col">
            ...
            <div className="p-4">
                ...
                {/* Lead name */}
                <h4 className="mb-1 text-lg font-bold text-[#101011]">{event.leadName}</h4>
                ...
                <div className="flex flex-col gap-3">
                    <div className="flex items-center gap-3 rounded-xl border border-[#F0F2F6] bg-[#FBFBFD] p-3">
                        ...
                        <div>
                            <p className="text-[10px] font-semibold uppercase text-[#9A9CA2]">Time</p>
                            <p className="text-sm font-medium text-[#101011]">
                                {formatHour(event.startHour)}{" "}
                                · {event.duration >= 1 ? `${event.duration}h` : `${event.duration * 60}m`}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3 rounded-xl border border-[#F0F2F6] bg-[#FBFBFD] p-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm font-medium text-[#101011]">{event.assignedTo}</p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3 rounded-xl border border-[#F0F2F6] bg-[#FBFBFD] p-3">
                        ...
                        <div>
                            <p className="text-[10px] font-semibold uppercase text-[#9A9CA2]">Status</p>
                            <span
                    ...
                    {event.amount ? (
                        <div className="flex items-center gap-3 rounded-xl border border-[#F0F2F6] bg-[#FBFBFD] p-3">
                            ...
                            <div>
                                ...
                                <p className="text-sm font-bold text-emerald-700">{event.amount}</p>
                            </div>
                        </div>
                    ) : null}
                </div>
