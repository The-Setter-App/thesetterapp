...
export default function ProfileSettingsContent({ user }: { user: AppUser }) {
  ...
  const [profileImageBase64, setProfileImageBase64] = useState(user.profileImageBase64 ?? '');
  const [savedDisplayName, setSavedDisplayName] = useState(normalizeDisplayName(user.displayName ?? ''));
  const [savedProfileImageBase64, setSavedProfileImageBase64] = useState(user.profileImageBase64 ?? '');
  const [saving, setSaving] = useState(false);
  const [success, setSuccess] = useState('');
  const [error, setError] = useState('');

  const normalizedDisplayName = useMemo(() => normalizeDisplayName(displayName), [displayName]);
  const hasUnsavedChanges = normalizedDisplayName !== savedDisplayName || profileImageBase64 !== savedProfileImageBase64;
  const canSave = normalizedDisplayName.length > 0 && !saving && hasUnsavedChanges;
  ...
  return (
    <div className="max-w-5xl space-y-4">
      ...
      {error ? (
        <div className="rounded-2xl border border-red-200 bg-red-50 px-5 py-3 text-sm font-medium text-red-700">{error}</div>
      ) : null}
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-[300px_minmax(0,1fr)]">
            <div className="space-y-4">
              <div className="rounded-2xl border border-[#F0F2F6] bg-[#F8F7FF] p-4">
                <p className="text-xs font-semibold uppercase tracking-wide text-[#606266]">Preview</p>
                <div className="mt-3 flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <p className="truncate text-sm font-semibold text-[#101011]">{normalizedDisplayName || 'Your name'}</p>
                    <p className="truncate text-xs text-[#606266]">{user.email}</p>
                  </div>
                </div>
              </div>

              <div className="rounded-2xl border border-[#F0F2F6] bg-white p-4">
                <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-[#606266]">Account details</p>
                <div className="space-y-2">
                  <div className="rounded-xl border border-[#F0F2F6] bg-[#F8F7FF] px-3 py-2">
                    ...
                    <p className="truncate font-mono text-xs text-[#101011]">{user.email}</p>
                  </div>
                  <div className="rounded-xl border border-[#F0F2F6] bg-[#F8F7FF] px-3 py-2">
                    <div className="mb-1 flex items-center gap-2 text-[#606266]">
                      <KeyRound size={14} />
            ...
            <div className="rounded-2xl border border-[#F0F2F6] bg-white p-4 md:p-5">
              <label htmlFor="display-name" className="mb-2 block text-sm font-medium text-[#101011]">
                Account name
              </label>
              <Input
                id="display-name"
              ...
              <div className="mt-4 rounded-xl border border-[#F0F2F6] bg-[#F8F7FF] p-3">
                ...
                <p className="mt-2 text-xs text-[#606266]">PNG/JPG up to {Math.floor(MAX_PROFILE_IMAGE_BYTES / 1_000_000)}MB</p>
              </div>
