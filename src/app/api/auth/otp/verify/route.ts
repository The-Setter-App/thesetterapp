...
import { upsertUser, verifyOTP } from "@/lib/userRepository";
...
export async function POST(request: Request) {
  try {
    ...
    const rateLimit = await enforceOtpVerifyRateLimit(
      normalizedEmail,
      clientIp,
    );
    ...
    const isValid = hasAccount
      ? await verifyOTP(normalizedEmail, normalizedOtp)
      : false;
    ...
    return NextResponse.json({
      success: true,
      requiresOnboarding: user.hasCompletedOnboarding === false,
    });
  } catch (error) {
