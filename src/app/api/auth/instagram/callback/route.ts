...
async function subscribePageToWebhooks(
  ...
  try {
    ...
    subscribeUrl.searchParams.append(
      "subscribed_fields",
      "messages,messaging_postbacks,messaging_optins,messaging_handovers",
    );
    ...
    return {
      ok: false,
      reason: error instanceof Error ? error.message : "subscribe_exception",
    };
  }
}
...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    return NextResponse.redirect(
      new URL(`/settings?error=${errorDescription || error}`, baseUrl),
    );
  }

  if (!code || !state) {
    return NextResponse.redirect(
      new URL("/settings?error=missing_code_or_state", baseUrl),
    );
  }
  ...
  if (!currentUser || currentUser.role !== "owner") {
    return NextResponse.redirect(
      new URL("/settings/team?error=owner_access_required", baseUrl),
    );
  }
  ...
  try {
    ...
    if (!tokenRes.ok) {
      throw new Error(
        `Failed to exchange code: ${tokenData.error?.message || "Unknown error"}`,
      );
    }
    ...
    if (!longLivedRes.ok) {
      throw new Error(
        `Failed to get long-lived token: ${longLivedData.error?.message}`,
      );
    }
    ...
    if (!permissionsRes.ok) {
      throw new Error(
        `Failed to fetch granted permissions: ${permissionsData?.error?.message || "unknown_error"}`,
      );
    }
      ...
      (permissionsData?.data || [])
        .filter(
          (p: { permission?: string; status?: string }) =>
            p?.status === "granted" && typeof p?.permission === "string",
        )
        .map((p: { permission: string }) => p.permission),
    );
    const missingScopes = REQUIRED_INSTAGRAM_SCOPES.filter(
    ...
    for (const page of pages) {
      ...
      pageDetailsUrl.searchParams.append(
        "fields",
      ...
      const instagramBusinessId = pageData?.instagram_business_account?.id as
        | string
        | undefined;
    ...
    if (accountConnections.length === 0) {
      const summary = diagnostics
        .map((d) => `${d.pageId}:${d.status}`)
        .join(",");
      const reason = summary || "no_pages_returned";
      return NextResponse.redirect(
        new URL(
          `/settings?error=no_instagram_business_account_found&reason=${encodeURIComponent(reason)}`,
          baseUrl,
        ),
      );
    }
    ...
    const failedSubscriptions = diagnostics.filter(
      (d) => d.status === "connected_webhook_subscribe_failed",
    );
    const warning =
      failedSubscriptions.length > 0
        ? `webhook_subscribe_failed_${failedSubscriptions.length}`
        : "";
    ...
    const message =
      error instanceof Error ? error.message : "instagram_oauth_failed";
