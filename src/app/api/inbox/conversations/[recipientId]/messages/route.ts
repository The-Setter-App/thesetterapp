...
export async function GET(
  ...
  try {
    ...
    let page = await getMessagesPageFromDb(
      conversation.id,
      workspaceOwnerEmail,
      limit,
      cursor,
    ...
    if (!cursor && page.messages.length === 0) {
      let account = conversation.accountId
        ? await getInstagramAccountById(
            workspaceOwnerEmail,
      ...
      if (!account) {
        const connectedAccounts =
          await getConnectedInstagramAccounts(workspaceOwnerEmail);
        if (conversation.ownerInstagramUserId) {
          account =
            connectedAccounts.find(
              (a) => a.instagramUserId === conversation.ownerInstagramUserId,
            ) || null;
        }
      ...
      if (account?.instagramUserId) {
        ...
        page = await getMessagesPageFromDb(
          conversation.id,
          workspaceOwnerEmail,
          limit,
          cursor,
