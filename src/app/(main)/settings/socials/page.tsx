      ...
      >
        <div className="border-b border-[#F0F2F6] px-6 py-6 md:px-8">
          <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="text-base font-bold text-[#101011]">Instagram Integrations</h3>
                <p className="mt-0.5 text-sm text-[#606266]">Sync Instagram Direct Messages from multiple accounts.</p>
              </div>
            </div>

            {isConnected ? (
              <Badge variant="secondary" className="gap-1.5 bg-[rgba(135,113,255,0.1)] px-3 py-1.5 text-sm text-[#8771FF]">
                <Link2 size={14} /> {accounts.length} Connected
              </Badge>
            ) : (
              <Badge variant="outline" className="gap-1.5 border-[#F0F2F6] bg-[#F8F7FF] px-3 py-1.5 text-sm text-[#606266]">
        ...
        {!isConnected && (
          <div className="flex flex-col items-center px-8 py-14 text-center">
            ...
            <p className="mb-2 text-base font-bold text-[#101011]">No Instagram accounts connected</p>
        ...
        {isConnected && (
          <div className="space-y-4 px-4 py-4 md:px-8">
            {accounts.map((account) => (
              <div key={account.accountId} className="rounded-2xl border border-[#F0F2F6] bg-[#F8F7FF] p-4 md:p-5">
                <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                  <div className="space-y-3">
                    <div className="flex items-center gap-2">
                      <Badge variant="outline" className="border-[#F0F2F6] bg-[rgba(135,113,255,0.1)] px-2 py-1 text-xs text-[#8771FF]">
                        Account
                      </Badge>
        ...
        <div className="flex justify-end border-t border-[#F0F2F6] bg-[#FAFAFF] px-6 py-6 md:px-8">
          <a href="/api/auth/instagram/login" className="inline-block w-full md:w-auto">
            <Button className="h-12 w-full bg-[#8771FF] text-white hover:scale-[1.02] hover:bg-[#6d5ed6] md:w-auto" leftIcon={<Plus size={16} />}>
